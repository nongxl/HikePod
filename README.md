...
- 过大的离线KML文件会加载不全，目前的限制是10000个路径点，将会继续优化
- 本应用的使用场景是在户外徒步时大多数时间代替手机定位以节省手机电量，使得手机可以保持与外界沟通，本应用不应替代专业定位设备
- 请不要在2800营地使用cardputer避免被神秘园网友捡到（笑.jpg）
...
- 电量消耗曲线不够平滑且会超出菜单范围
- 增加USB模式方便存储kml文件
- GPS info模式排版优化（正在进行中）
- GPS info模式显示的时间根据定位信息自动计算本地时区显示（已实现，待测试）

### 后续还考虑实现
1. **加载更大的kml文件**
   - 将double改成float类型，减少内存占用
   - 自动抽样，减少路径点数量（自动计算每隔多少个点丢弃一个，而不是每隔多少个点取一个）
2. **省电优化**
   - 需要进一步观察效果。在设置菜单下增加电量消耗情况曲线
   - 徒步一天7小时，还剩30%电量，可以接受了。还有优化点后续再做
3. **加载离线地图背景**
   - 可能内存不够，目前加载3MB的kml文件已经是经过优化了
...
- **[Space]**: Lock/unlock current position (2D) / Toggle rotation center (3D)
- **[t]**: Start/stop track recording mode
- **[v]**: Switch to 3D view
- **[TAB]**: Switch to GPS Info mode
- **[ESC]**: Toggle debug information display
- **[+/-]**: Zoom in/out map
- **[[/]]**: Adjust 3D view vertical exaggeration
- **[Arrow keys]**: Pan map (2D) / view (3D)
...
- Large offline KML files may not load completely, current limit is 10000 path points, will continue to optimize
...
- Please do not use cardputer at 2800 camp to avoid being picked up by mysterious garden netizens (laugh.jpg)
...
- Add USB mode for convenient KML file storage
- GPS info mode layout optimization (in progress)
- GPS info mode time display automatically calculates local timezone based on location (implemented, pending test)

### Future Plans
1. **Loading Larger KML Files**
   - Change double to float type to reduce memory usage
   - Automatic sampling to reduce path point count (automatically calculate how many points to discard, rather than taking one every N points)
...
3. **Loading Offline Map Background**
   - May not have enough memory, currently loading 3MB kml files is already optimized
...
6. **Add USB Mode**
   - Can connect to PC via USB for convenient KML file storage
7. **3D Path Rendering**
   - Draw 3D path map with latitude/longitude + altitude (in progress)
